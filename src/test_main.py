...
def video_playback(video_file):
    global latest_frame, running
    cap = cv2.VideoCapture(video_file)
    # Let the video play at its natural speed:
    video_fps = cap.get(cv2.CAP_PROP_FPS)
    delay = 1.0 / video_fps if video_fps > 0 else 1/30
    while running:
        ret, frame = cap.read()
        if not ret:
            break
...
def main():
    ...
    # List of videos to simulate a live stream
    ...
    for video_file in video_files:
        ...
        # Start video playback thread (runs continuously at the video's natural pace)
        video_thread = threading.Thread(target=video_playback, args=(video_file,))
        # Start CV inference thread (picks up the latest frame when ready)
        inference_thread = threading.Thread(target=cv_inference_worker, args=(interpreter, input_details, output_details, input_size, csv_writer, video_file, sim_start_time))
